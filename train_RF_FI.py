...
if __name__ == "__main__":
    ...
    if args.rfe:
        ...
        fitting1 = rf.fit(x_train, y_train)
        feature_importances = rf.feature_importances_
        selected_features = feature_importances.argsort()[::-1][:20]  # Select top 20 features
        selected_features_mask = [True if i in selected_features else False for i in range(len(feature_importances))]

        x_train = x_train.loc[:, selected_features_mask]
        x_val = x_val.loc[:, selected_features_mask]
        x_test = x_test.loc[:, selected_features_mask]

        dump(selected_features_mask, f'Models/RF_FI/{IBSorOBS}_{args.tune}_{ds_period}_{ds_duration}_features.joblib')
        ...
        # save the mapper
        dump(mapper, f"Models/RF_FI/{IBSorOBS}_{args.tune}_{ds_period}_{ds_duration}_scaler.joblib")
            
        # save the model
        dump(search, f"Models/RF_FI/{IBSorOBS}_{args.tune}_{ds_period}_{ds_duration}.joblib")